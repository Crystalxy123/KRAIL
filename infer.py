...
def update_base_HEPs(scen_task,scen_context,scen_cog,scen_time,table1,table2,relationship,data_source, model,temperature):
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + "请你从"+str(PIF_Attribute_lib)+f"中选择最适合的输出PIF attribute,输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_PIF_Attri}。"+"请严格按照要求格式进行输出。" }]
...
def update_info(scen_task,scen_context,scen_cog,scen_time,object_input,result_label,data_source, model,temperature):
    if object_input == "PIF Attributes and Base HEPs":
        if result_label == "Scenario Familiarity":
            ...
            pass
        task_error_measure,cfms,relevent_pif,PIF_measure,other_pif_uncertainties = update_base_HEPs(scen_task,scen_context,scen_cog,scen_time,table1,table2,relationship,data_source, model,temperature)
...
def process(value_data_source):
    ...
    task_error_measure,cfms,relevent_pif,other_pif_uncertainties,PIF_measure,table2,table1,relationship,task_lib_list,CFMs_lib_list,other_pif_lib_list,PIFs_measure_lib_list,PIF_Attribute_lib_list=update_info(scen_task,scen_context,scen_cog,scen_time,object_input_state,result_label_state,data_source,model,temperature)
    return task_error_measure,cfms,relevent_pif,other_pif_uncertainties,PIF_measure,table2,table1,relationship,task_lib_list,CFMs_lib_list,other_pif_lib_list,PIFs_measure_lib_list,PIF_Attribute_lib_list
       
def process_row(i, row,df_columns,output_dir):
    # 提取前六列的内容
    ...
    # 获取处理结果
    result = process(concatenated_values)
...
if __name__ == "__main__":
    ...
    i=0
    output_dir=f"/Users/jyxc-dz-0100458/Downloads/HRA/test/output2/{args.dir}"

    if args.parall=="True":
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = list(executor.map(process_row, range(len(df)), [df.iloc[i] for i in range(len(df))], [df.columns] * len(df), [output_dir] * len(df)))
        ...
        for idx, row in df.iterrows():
            ...
            out_path=f"/Users/jyxc-dz-0100458/Downloads/HRA/test/output2/data{i}.json"
            # 保存到JSON文件
            with open(out_path, 'w', encoding='utf-8') as json_file:
                json.dump(output_row_json, json_file, ensure_ascii=False, indent=4)
            # 将结果添加到output_data中
