...
def update_base_HEPs(scen_task,scen_context,scen_cog,scen_time,table1,table2,relationship,data_source, model,temperature,df,i):
    ...
    learn_task="已知对于任务"+str(rest_cols1)+"的Task (and error measure)为"+str(front_cols1.tolist()[3])+\
    "已知对于任务"+str(rest_cols2)+"的Task (and error measure)为"+str(front_cols2.tolist()[3])+\
    "已知对于任务"+str(rest_cols3)+"的Task (and error measure)为"+str(front_cols3.tolist()[3])

    learn_measure="已知对于任务"+str(rest_cols1)+"的PIF Measure为"+str(front_cols1.tolist()[4])+\
    "已知对于任务"+str(rest_cols2)+"的PIF Measure为"+str(front_cols2.tolist()[4])+\
    "已知对于任务"+str(rest_cols3)+"的PIF Measure为"+str(front_cols3.tolist()[4])
