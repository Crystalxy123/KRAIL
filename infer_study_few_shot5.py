...
def similar(data_source,df,i):
    ...
    # 找出最相似的行号
    top_5_similar_rows = similarities.nlargest(5)
    
    # Get the front 6 columns and the concatenated rest columns for each of the top 3 rows
    best_matches = []
    for row_idx in top_5_similar_rows.index:
        best_match_front_cols = df.iloc[row_idx, :6]
        best_match_rest_cols = df.iloc[row_idx, 6:].astype(str).str.cat()
        best_matches.append((best_match_front_cols, best_match_rest_cols))
...
def update_base_HEPs(scen_task,scen_context,scen_cog,scen_time,table1,table2,relationship,data_source, model,temperature,df,i):
    best_matches = similar(data_source,df,i)
    print(f" df shape: {df.shape}")

    front_cols1, rest_cols1 = best_matches[0]
    front_cols2, rest_cols2 = best_matches[1]
    front_cols3, rest_cols3 = best_matches[2]
    front_cols4, rest_cols4 = best_matches[3]
    front_cols5, rest_cols5 = best_matches[4]

    learn_pif="已知对于任务"+str(rest_cols1)+"的PIF为"+str(front_cols1.tolist()[0])+\
                "已知对于任务"+str(rest_cols2)+"的PIF为"+str(front_cols2.tolist()[0])+\
                "已知对于任务"+str(rest_cols3)+"的PIF为"+str(front_cols3.tolist()[0])+\
                "已知对于任务"+str(rest_cols4)+"的PIF为"+str(front_cols4.tolist()[0])+\
                "已知对于任务"+str(rest_cols5)+"的PIF为"+str(front_cols5.tolist()[0])
    
    learn_CFM="已知对于任务"+str(rest_cols1)+"的CFM为"+str(front_cols1.tolist()[1])+\
    "已知对于任务"+str(rest_cols2)+"的CFM为"+str(front_cols2.tolist()[1])+\
    "已知对于任务"+str(rest_cols3)+"的CFM为"+str(front_cols3.tolist()[1])+\
    "已知对于任务"+str(rest_cols4)+"的CFM为"+str(front_cols4.tolist()[1])+\
    "已知对于任务"+str(rest_cols5)+"的CFM为"+str(front_cols5.tolist()[1])

    learn_task="已知对于任务"+rest_cols1+"的Task (and error measure)为"+front_cols1.tolist()[3]+\
    "已知对于任务"+rest_cols2+"的Task (and error measure)为"+front_cols2.tolist()[3]+\
    "已知对于任务"+rest_cols3+"的Task (and error measure)为"+front_cols3.tolist()[3]+\
    "已知对于任务"+rest_cols4+"的Task (and error measure)为"+front_cols4.tolist()[3]+\
    "已知对于任务"+rest_cols5+"的Task (and error measure)为"+front_cols5.tolist()[3]

    learn_measure="已知对于任务"+rest_cols1+"的PIF Measure为"+front_cols1.tolist()[4]+\
    "已知对于任务"+str(rest_cols2)+"的PIF Measure为"+front_cols2.tolist()[4]+\
    "已知对于任务"+rest_cols3+"的PIF Measure为"+front_cols3.tolist()[4]+\
    "已知对于任务"+rest_cols4+"的PIF Measure为"+front_cols4.tolist()[4]+\
    "已知对于任务"+rest_cols5+"的PIF Measure为"+front_cols5.tolist()[4]

    learn_other="已知对于任务"+str(rest_cols1)+"的Other PIFs (and Uncertainty)为"+str(front_cols1.tolist()[5])+\
    "已知对于任务"+str(rest_cols2)+"的Other PIFs (and Uncertainty)为"+str(front_cols2.tolist()[5])+\
    "已知对于任务"+str(rest_cols3)+"的Other PIFs (and Uncertainty)为"+str(front_cols3.tolist()[5])+\
    "已知对于任务"+str(rest_cols4)+"的Other PIFs (and Uncertainty)为"+str(front_cols4.tolist()[5])+\
    "已知对于任务"+str(rest_cols5)+"的Other PIFs (and Uncertainty)为"+str(front_cols5.tolist()[5])
    ...
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + f"请你从{task_lib}中选择最适合的输出Task (and error measure),error measure在()里面，输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_task}。"\
            +learn_task+"\n"+"请严格按照要求格式进行输出。" }]
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + f"请你从{CFMs_lib}中选择最适合的输出CFMs,输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_CFMs}。"\
            +learn_CFM+"\n"+"请严格按照要求格式进行输出。" }]
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + "请你从"+str(PIF_Attribute_lib)+f"中选择最适合的输出PIF attribute,输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_PIF_Attri}。"+\
            learn_pif+"\n""请严格按照要求格式进行输出。" }]
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + f"请你从{PIF_Measure_lib}中选择最适合的输出PIF attribute measure,输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_PIF_measure}。"\
            +learn_measure+"\n"+"请严格按照要求格式进行输出。" }]
        ...
        {"role": "user", "content": 
        "请以如下的信息要求来撰写："  + f"请你从{Other_pif_lib}中选择最适合的输出Other PIFs (and Uncertainty),（）里面的内容是不确定性的内容，输出分析过程，最终结果输出在<>之间，用英文回答我的问题，如{example_other_PIF_uncertainties}。"\
            +learn_other+"\n"+"请严格按照要求格式进行输出。" }]
